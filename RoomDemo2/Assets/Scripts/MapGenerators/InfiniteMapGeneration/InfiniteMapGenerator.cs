...
public class InfiniteMapGenerator : MonoBehaviour
{
    ...
    void GenerateGrid()
    {
        ...
        for (int row = 0; row < gridRows; row++)
        {
            for (int col = 0; col < gridCols; col++)
            {
                ...
                // Create and save RoomData
                RoomData roomData = new RoomData
                {
                    prefabName = selectedRoomPrefab.name,
                    position = position,
                    rotation = rotation,
                    hasTopWall = true,
                    hasBottomWall = true,
                ...
                if (TryGenerateChest(room)) roomData.containsChest = true;
                if (TryGenerateHerbs(room)) roomData.containsHerbs = true;
                if (TryGenerateStone(room)) roomData.containsStones = true;

                roomDataList.Add(roomData);
            }
        }

        // Remove overlapping walls and update RoomData
        RemoveOverlappingWalls(grid);
        SaveRoomData("roomData.json");
    }

    void SaveRoomData(string filePath)
    {
        try
        {
            RoomDataWrapper wrapper = new RoomDataWrapper { rooms = roomDataList };
            string json = JsonUtility.ToJson(wrapper, true);
            System.IO.File.WriteAllText(filePath, json);
            Debug.Log($"Room data successfully written to {filePath}");
        }
        catch (Exception ex)
        {
            Debug.LogError($"Error writing room data: {ex.Message}");
        }
    }
    ...
    void RemoveOverlappingWalls(GameObject[,] grid)
    {
        for (int row = 0; row < gridRows; row++)
        {
            for (int col = 0; col < gridCols; col++)
            {
                ...
                RoomData roomData = roomDataList.Find(data => data.position == room.transform.position);
                ...
                if (col < gridCols - 1 && grid[row, col + 1] != null)
                {
                    ...
                    if (roomData != null) roomData.hasRightWall = false;

                    RoomData neighborData = roomDataList.Find(data => data.position == grid[row, col + 1].transform.position);
                    if (neighborData != null) neighborData.hasLeftWall = false;
                }
                ...
                if (row < gridRows - 1 && grid[row + 1, col] != null)
                {
                    ...
                    if (roomData != null) roomData.hasTopWall = false;

                    RoomData neighborData = roomDataList.Find(data => data.position == grid[row + 1, col].transform.position);
                    if (neighborData != null) neighborData.hasBottomWall = false;
                }
            }
        }
    }
    ...
    bool TryGenerateChest(GameObject room)
    {
    ...
    bool TryGenerateHerbs(GameObject room)
    {
    ...
    bool TryGenerateStone(GameObject room)
    {
        bool hasStone=false;
        for (int i = 0; i < 5; i++)
        {
